...
# 5.8 ====================================
from transformers import BertModel, BertConfig
config = BertConfig.from_pretrained('data/config.json')
model = BertModel.from_pretrained('data/pytorch_model.bin', config=config)

config = BertConfig.from_pretrained('data/config.json')
model = BertForMaskedLM.from_pretrained('data/pytorch_model.bin', config=config)

from transformers import BertJapaneseTokenizer
tknz = BertJapaneseTokenizer.from_pretrained(
    vocab_file='data/vocab.txt',do_lower_case=False,do_casic_tokenize=False)
...
model = BertModel.from_pretrained('laboro.bin', config)
a = model(x)
print(a[0].shape)
